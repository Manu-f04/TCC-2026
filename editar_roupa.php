...
<body class="index-page">
   ...
   <script type="module">
    ...
    // Função global para responder ao evento onchange do input de foto
    window.previewImg = async function(input) {
        ...
        // 1. MOSTRA A IMAGEM ORIGINAL NA TELA IMEDIATAMENTE
        if (currentImg) currentImg.style.display = 'none';
        prev.src = URL.createObjectURL(originalFile);
        prev.classList.remove('d-none');

        // Cria/Recupera elemento de texto de status abaixo da imagem
        let statusText = document.getElementById('bgStatusText');
        if (!statusText) {
            statusText = document.createElement('div');
            statusText.id = 'bgStatusText';
            statusText.className = 'small text-muted mt-2 fw-bold';
            prev.parentNode.appendChild(statusText);
        }
        ...
        try {
            // 2. PROCESSA A REMOÇÃO DE FUNDO EM SEGUNDO PLANO (modelo leve 'small')
            const blob = await removeBackground(originalFile, {
                model: 'small', 
                progress: (key, current, total) => {
                    if (total > 0) {
                        const percent = Math.round((current / total) * 100);
                        statusText.innerText = `⏳ Baixando modelo de IA: ${percent}%...`;
                    }
                }
            });

            // 3. ATUALIZA A IMAGEM NA TELA PARA A VERSÃO SEM FUNDO
            const newUrl = URL.createObjectURL(blob);
            prev.src = newUrl;
            statusText.className = 'small text-success mt-2 fw-bold';
            statusText.innerText = '✅ Fundo removido com sucesso!';

            // Substitui o arquivo no <input> para que o formulário PHP envie o PNG sem fundo
            const nomeSemExt = originalFile.name.substring(0, originalFile.name.lastIndexOf('.')) || originalFile.name;
            const novoArquivo = new File([blob], `${nomeSemExt}_sem_fundo.png`, { type: "image/png" });
            ...
            // Em caso de erro/falha de conexão, mantém a imagem original
            statusText.className = 'small text-warning mt-2 fw-bold';
            statusText.innerText = '⚠️ Não foi possível remover o fundo. Mantida foto original.';
        } finally {
